...
function consultar(semCodigo){
  audioAviso.volume = 0.7;
  audioBomAlmoco.volume = 0.7;
  audioErro.volume = 0.7
    ...
    if(data.tipo == 1){
      ...
      if(data.sucesso == 1){
        ...
        if(data.isTicket == 1){
          ...
          vartudoNone = 1;
          $.getJSON("http://localhost/liberaCatraca.php",function(data){
            verTudoNone(1);
          });
          ...
          if(data.aniversario == 1){
            ...
            $("#status #tudo h1").html("BOM ALMOÇO!<br><b>FELIZ ANIVERSÁRIO!!!</b>");
            $("#fundo").addClass("bgPurple");
          }else{
            $("#status #tudo h1").html("BOM ALMOÇO!");
            $("#fundo").addClass("bgGreen");
          }
