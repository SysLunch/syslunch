<?php
  ...
  $factory->importaClasses(array("login" => 1, "empresa" => 1, "tipoLetra" => 1));
  ...
  <table class="CRTConfigTable">
      <tbody>
          ...
          <tr>
            <td>Tipo de Letra</td>
            <td>
              <?php $letras = $factory->getObjeto("tipoLetra")->listaLetras(1,1); ?>
              <select id="tipoLetra">
                <optgroup label="Qual letra será usada?"></optgroup>
                <?php foreach($letras as $inf){ ?>
                <option value="<?php echo $inf["idLetra"] ?>" <?php if($dadosEmpresa["idLetra"] == $inf["idLetra"]){ ?> selected="selected"<?php } ?>><?php echo $inf["letraReserva"] ?>: <?php echo $inf["descricaoTReserva"] ?> - <?php echo ($inf["isGratuito"] == 1) ? "Gratuito" : "Pago"; ?></option>
                <?php } ?>
              </select>
            </td>
          </tr>
