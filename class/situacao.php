...
class Situacao{
  ...
  function buscaSituacaoporNome($nomeSituacao){
    global $factory;
    $conexao = $factory->getObjeto("conexao")->getInstance();

    $sql = "SELECT * FROM situacao WHERE nomeSituacao = :p1;";
    $consulta = $conexao->prepare($sql);
    $consulta->bindParam(":p1", $nomeSituacao);
    $consulta->execute();
  ...
  function buscaSituacaoporNomeparaLabel($nomeSituacao){
    global $factory;
    $conexao = $factory->getObjeto("conexao")->getInstance();
    ...
    if($consulta->rowCount() > 0){
      ...
      if($return[0]["classesIcon"] != NULL){
        return array("count" => 1, "consulta" => "<span class='label ".$return[0]["tipoTag"]."'><i class='".$return[0]["classesIcon"]."'></i> ".$return[0]["nomeSituacao"]."</span>");
      }else{
        return array("count" => 1, "consulta" => "<span class='label ".$return[0]["tipoTag"]."'>".$return[0]["nomeSituacao"]."</span>");
      }
    }else{
      return array("count" => 0);
    }
  ...
  function buscaSituacao($idSituacao){
    global $factory;
    $conexao = $factory->getObjeto("conexao")->getInstance();

    $sql = "SELECT * FROM situacao WHERE idSituacao = :p1;";
    $consulta = $conexao->prepare($sql);
    $consulta->bindParam(":p1", $idSituacao);
    $consulta->execute();
    if($consulta->rowCount() > 0){
      return array("count" => 1, "consulta" => $consulta->fetchAll());
    }else{
  ...
  function buscaSituacaoporIdparaLabel($idSituacao){
    global $factory;
    $conexao = $factory->getObjeto("conexao")->getInstance();
  ...
  function buscaEmpresa($id, $ativo = 2){
      global $factory;
      $conexao = $factory->getObjeto("conexao")->getInstance();

      if($ativo == 2){
        $sql = "SELECT * FROM empresa WHERE idEmpresa = :p1;";
      }else{
        $sql = "SELECT * FROM empresa WHERE idEmpresa = :p1 && ativoEmpresa = :p2;";
      }
      $consulta = $conexao->prepare($sql);
